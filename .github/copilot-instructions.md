...
- DB: `init_db()` creates `users`, `zones`, `audit_log`; soft-delete via `deleted_at`; foreign keys on; stored at `alerte_parapente.db`.
...
- Locks: 15-minute lock via `POST /zones/{id}/checkout`; PUT requires matching `locked_by`; PUT auto-clears lock; manual release via `POST /zones/{id}/release`; expired lock yields 409; lock info exposed at `GET /zones/{id}/lock`.
- Geometry rules: Shapely validation; only Polygon/MultiPolygon; intersection guard erodes both geometries by ~10cm and blocks overlaps above tiny epsilon (see `_geometry_intersects_existing` in [app/api/zones.py](app/api/zones.py)).
...
- Real-time: Socket.IO events `zone_created|updated|deleted|locked|released`; backend injects `sio` via `set_sio`; `ws_manager` tracks `sid -> user` mapping; engine logs enabled for debugging.
- Frontend state: `AppState` handles modes VIEW/DRAW/EDIT, selected zone, lockStatus polling every 5s, quota cache (see [static/js/app-state.js](static/js/app-state.js)).
...
- Frontend realtime: `SOCKET` loader in [static/js/socket.js](static/js/socket.js) dynamically loads client lib, auto-authenticates with `auth_user`, and falls back to zone polling if disconnected.
- UI patterns: drawer shows details or edit form; quota panel fed by `/auth/quota`.
- Zones flow: select → checkout → edit geometry/zone_type/description copy → PUT update → lock released and broadcast; deletions are soft and broadcast.
- Volunteers: fonctionnalités de périmètres supprimées; aucun endpoint `/volunteers`.
- Debug tips: middleware logs 4xx/5xx in [app/main.py](app/main.py); inspect locks with `sqlite3 alerte_parapente.db "SELECT id, locked_by, lock_expires_at FROM zones;"`; quotas visible via `/auth/quota`.
...
- Keep soft-delete filters (`WHERE deleted_at IS NULL`) and lock checks on all mutating operations; prefer broadcasting `zone_*` events after DB commit.
