...
- Forme de l’API : réponses `{success, data, error?}` ; attend du GeoJSON Polygon/MultiPolygon ; codes `zone_type` `DIFFICULT_ACCESS`/`REMOTE_AREA`/`TAKEOFF`/`LANDING`/`PREPARATION_ZONE` (voir [static/js/ui.js](static/js/ui.js)).
