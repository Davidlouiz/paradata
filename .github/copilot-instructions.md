...
- DB: `init_db()` creates `users`, `map_objects`, `audit_log`, `volunteer_coverage`; soft-delete via `deleted_at`; foreign keys on; stored at `alerte_parapente.db`.
...
- UI patterns: drawer shows details or edit form; coverage sheet disables create button; quota panel fed by `/auth/quota`; volunteers coverage list driven by `/volunteers/coverage` endpoints (see [static/js/ui.js](static/js/ui.js)).
...
- Volunteers: endpoints under `/volunteers` for listing/adding/deleting coverage and fetching covering volunteers for an object; UI highlights coverage on map.
...
- Tests to sanity-check: create/edit/delete polygon with lock conflicts, quota exhaustion, grace delete (within 120 minutes), overlap rejection, lock expiry conflict, volunteer coverage display refresh.
